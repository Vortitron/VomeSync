...
- **Features**:
  ...
  - **Health score, with no account:** the `vomesync.health_score_run` action checks this Home Assistant — noisy sensors, a swelling database, dead entities, backups that stopped — and puts the score and every finding on `sensor.vome_health_score`. If this instance is not linked to Vome yet it links itself temporarily, so there is nothing to sign up for and nothing to type; Vome deletes that run after a day unless you sign in from the link it gives you, and the report stays here either way. The add-on panel, the notification and the sensor all carry an **Open this score online** link (plus AI Doctor and share-card links once it is saved). Hosting and Connect customers publish the 49 kr shareable card for free. **Chatty devices** is counted from the recorder database in this integration (`GET /api/vomesync/health/recorder_counts`), not a 24 h history dump — update to 0.9.33 / add-on 0.3.38 and restart Home Assistant or that check will keep failing on a busy recorder. See [docs/HEALTH_SCORE.md](docs/HEALTH_SCORE.md).
  - **Remote address, with no account:** *Configure → Get a remote address* (or `vomesync.get_remote_address`) issues a random `*.home.vome.io`. Unlinked, that is the same guest call as the health score; already linked, it asks `POST /api/sync/agent/remote-address` so a device-code house still gets the free URL. Point the Home Assistant app at it. No domain, no router ports. Signing in keeps the address. Full-UI forwarding is switched on for a relay guest so the app can actually reach Core.
...
- **Premium:** 50 switches / 25 public, via Stripe Checkout (€9 / month including VAT), promo code, or admin grant. The staff catalogue owner is exempt. Cancel or change card from the switch page or **More → Manage billing** in Home Assistant.
